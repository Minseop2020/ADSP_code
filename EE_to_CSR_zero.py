...
def add_gaussian_noise_to_csr(csr_data, percentage, noise_ratio):
    ...
    num_noisy_elements = int(dense_data.size * percentage / 100)
    print("num_noisy_elements:")
    print(num_noisy_elements)
    
    # Randomly select indices of the elements to add noise to
    noisy_indices = np.unravel_index(
        np.random.choice(dense_data.size, num_noisy_elements, replace=False),
        dense_data.shape
    )
    
    # Generate Gaussian noise
    noise = np.random.normal(loc=0, scale=noise_ratio, size=num_noisy_elements)
    print("noise:")
    print(noise)
    
    # Add Gaussian noise to the selected elements
    dense_data[noisy_indices] += noise

    
    # Convert the noisy dense matrix back to CSR format
    noisy_csr = csr_matrix(dense_data)
...
percentage = 1  # Percentage of elements to add noise to
...
save_csr_to_mtx(csr_noisy, 'C:\\Work\\School\\Coding\\494_bus\\494_bus_noisy_zero.mtx')
